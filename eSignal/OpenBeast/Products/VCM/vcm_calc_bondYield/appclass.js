var UserID_vcm_calc_bondYield = "1";
var CustomerID_vcm_calc_bondYield = "2";
var instanceMode_vcm_calc_bondYield = "conn";
var instanceType_vcm_calc_bondYield = "vcm_calc_bondYield";

F2.Apps["vcm_calc_bondYield"] = (function () {
    ...
    var App_Class = function (appConfig, appContent, root) {
        ...
        if (appConfig.context != null) {
            UserID_vcm_calc_bondYield = appConfig.context.UserID;
            CustomerID_vcm_calc_bondYield = appConfig.context.CustomerID;
            instanceMode_vcm_calc_bondYield = appConfig.context.InstanceMode;
        }
        ...
        App_Class.prototype.init = function () {
            ...
            $('#vcm_calc_bondYield :text[title="datepick"]').datepicker({ format: 'mm/dd/yyyy', autoclose: true, weekStart: 0 }).on('changeDate', function (ev) {
                try {
                    //alert("abc" + ev.date);
                    var paraValues = UserID_vcm_calc_bondYield + "^" + CustomerID_vcm_calc_bondYield + "^" + instanceMode_vcm_calc_bondYield;
                    ...
                    if ($(this).val() != "")
                        SendToBeast(instanceType_vcm_calc_bondYield + "#" + paraValues, idValPair);
                }
                catch (err) {
                    var strerrordesc = "Function:datepick(); Error is : " + err.description + "; Error number is " + err.number + "; Message :" + err.message;
                    onJavascriptLog("vcm_calc_bondYield_appclass.js", strerrordesc);
                }
            });

            $('#vcm_calc_bondYield :text').click(function () {
                debugger;
                if ($(this).hasClass("priceWidget")) {
                    try {
                        ...
                        var eleInfo = instanceType_vcm_calc_bondYield + "^" + paraValues + "^" + idValPair;
            ...
            $("#vcm_calc_bondYield input[type='button']").click(function () {
            ...
            $("#vcm_calc_bondYield select").change(function () {
                try {
                    var itemType = "DDList";
                    var paraValues = UserID_vcm_calc_bondYield + "^" + CustomerID_vcm_calc_bondYield + "^" + instanceMode_vcm_calc_bondYield;
                    var idValPair = itemType + "#" + $(this).attr("id").substring($(this).attr("id").lastIndexOf('_') + 1) + "#" + $(this).val();
            ...
            $("#vcm_calc_bondYield :text").bind("paste", function (e) {
                e.preventDefault();
            });

            $("#vcm_calc_bondYield :text").bind('keydown', function (event) {
                try {
                    if ($(this).hasClass("inputDisable")) {
